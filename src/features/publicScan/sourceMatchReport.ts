...
export function renderSourceMatchReport(result: ScanResult, decisions: Record<string, ScanDecision>, context: ReportContext, exportedAt = new Date().toISOString()) {
  ...
  const revisionHtml = upload
    ? link(result.repository.commitUrl, result.repository.commit || "Not recorded")
    : result.repository.revision?.kind === "none"
      ? "No Git commit exists — repository is empty"
      : link(result.repository.commitUrl, result.repository.commit || "Not recorded");
...
<section><h2>What was scanned</h2><dl><dt>Input</dt><dd>${escape(result.source?.type === "zip" ? "Uploaded ZIP" : result.source?.type === "files" ? "Selected local folder files" : "Public GitHub repository")}</dd><dt>Project</dt><dd>${link(result.repository.url, result.repository.name)}</dd><dt>${upload ? "Content digest (not a Git commit)" : "Git revision"}</dt><dd class="mono">${revisionHtml}</dd></dl></section>
